...
import { Container, Header, Content, Footer, Grid, Divider, Row, Col } from 'rsuite';
...
function CompanyOfficialList(props) {
  ...
  const handlerOfficialChange = useCallback(updatedOfficial => {
    const indexOfUpdatedOfficial = data.findIndex(official => official.id === updatedOfficial.id)

    if (updatedOfficial.member && updatedOfficial.designation) { // update
  ...
  return (
    <Container className="Official-list">
      <Header className="Input-group-title" style={{ marginBottom: 25 }}>
        Company Officials
        <h4 style={{ color: '#0f203c', marginTop: 10 }}>Strengthen your company page by adding your leadership team, founders, and brand ambassadors</h4>
      </Header>
      ...
      <Content>
        <Grid fluid style={{ padding: 0 }}>
          ...
          {(data.length > 0) && data.map((official, index) => {
            return (
