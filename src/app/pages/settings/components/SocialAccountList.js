...
function SocialAccountList(props) {
  ...
  const handlerSocialAccountChange = useCallback(
    (oldType, newType, url, isRemoved = false) => {
      if (isRemoved) {
        ...
        const indexOfOldSocial = data.findIndex(
          social => social.value === oldType
        );
        data[indexOfOldSocial].isOpened = false;
        const indexOfFirstUnOpenedSocial =
          unOpenedSocials.length > 0
            ? data.findIndex(
                social => social.value === unOpenedSocials[0].value
              )
            : data.length;
        ...
        // update accountURL
        const indexOfOldSocial = data.findIndex(
          social => social.value === newType
        );
        data[indexOfOldSocial].url = url;
      } else {
        // switch other social type
        // new
        const indexOfNewSocial = data.findIndex(
          social => social.value === newType
        );
  ...
  return (
    <Container className="Social-list">
      <Header className="Input-item-title">Social Media Accounts</Header>
        ...
        {openedSocials.map((social, index) => {
          return (
            <SocialAccountListItem
              ...
              onRemoved={oldType =>
                handlerSocialAccountChange(oldType, null, null, true)
              }
